...
### 11.5 File storage
- Resume/document uploads (resumes, org documents, lesson uploads) already only persist AI-extracted text in Postgres `Text` columns — the raw uploaded bytes are read once for extraction and discarded, never stored. No database bloat risk there. **Done, no action needed.**
- Object storage (Cloudflare R2, S3-compatible) added for the one place that *did* need it: profile avatars, previously written to local disk (`uploads/avatars/`), which is not viable on Vercel's ephemeral serverless filesystem. `backend/app/storage.py` — falls back to local disk automatically when `R2_*` env vars are unset, so dev works with zero setup. **Done**, needs `R2_ACCOUNT_ID`/`R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY`/`R2_BUCKET`/`R2_PUBLIC_URL` (free Cloudflare R2 signup) to activate in production.
