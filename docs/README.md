...
2. Free/cheap signups still needed, each activating an already-built feature: `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` (OAuth login), `STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET` (credit purchases), `R2_ACCOUNT_ID`/`R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY`/`R2_BUCKET`/`R2_PUBLIC_URL` (avatar object storage — has a local-disk fallback for dev). `SENTRY_DSN` is set locally but still needs adding to Vercel's production env vars to activate error tracking there too.
3. A billed Google Cloud account before real user traffic — the current Gemini key is free-tier and will not survive concurrent load. (Owned by the client, not a dev task.)
4. Once `npm install` is resolved: click through the entire Exhibit A flow live in a browser for the first time.
5. Confirm NanTechs actually has GitHub admin access on this repo (not a code question, needs manual check).

**Already done and live-verified:** both exposed secrets (Gmail app password, Neon DB password) rotated and tested against the real SMTP/Neon endpoints; Upstash Redis configured and tested against the real instance (`is_redis_configured() == True`, live set/get + atomic incr round trip); migrated off the deprecated `google.generativeai` SDK to `google-genai`; added a 27-test automated pytest suite (previously zero tests existed anywhere in the repo); **encryption at rest is now active** (`ENCRYPTION_KEY` generated and set, live-verified with a real encrypt/decrypt round trip — resume text and org-document text are no longer stored as plaintext); **Sentry error tracking is active locally** (`SENTRY_DSN` set, live-verified with a real test event sent and flushed to the dashboard); **Adzuna job search is live** (`ADZUNA_APP_ID`/`ADZUNA_APP_KEY` set, live-verified against the real API and through the actual `/api/jobs/search` endpoint — 20 real results returned); **YouTube course-matching fallback is live** (`YOUTUBE_API_KEY` set, live-verified against the real API and through `/api/video/search`); added object storage for avatars (built, awaiting R2 credentials to activate); closed an unauthenticated endpoint (`video.py` search) and a hardcoded `SECRET_KEY` fallback vulnerability found during a full-codebase audit against the Exhibit A and Development Agreement source PDFs; production Vercel env vars fully rebuilt from local `.env` (all 30 keys, stale/unused entries like `DATABASE_URL_UNPOOLED` dropped) — live-verified post-deploy via `/api/health` (200, healthy) and `/api/auth/me` (401 with no token, correct behavior, not a crash); full backend + read-only frontend cleanup pass — consolidated the credit-gated-AI-call pattern (10 duplicated sites across 6 router files) and the 3 Gemini wrapper functions into shared helpers, removed dead code (an unused `Certification` model/table, a `UserLogin` schema, several unused imports, a dead frontend YouTube-classification function), all verified against the 27-test suite plus live Gemini calls at each step.
