...
1. **`npm install` — real root cause found, not yet resolved.** It was never memory pressure. Windows Defender has been silently killing `node.exe` on every attempt since July 26 (confirmed via Defender's detection history), blocking a malicious script (`Trojan:Win32/NpmSteal.MU!MTB`, a known npm-supply-chain credential-stealer pattern) before it could run. Isolated the trigger to `motion-dom` (pulled in transitively by `framer-motion`) — installing it alone reproduces the failure 100% of the time, across multiple versions, independent of `--ignore-scripts` and `--no-audit`. Was mid-way through inspecting the actual downloaded tarball contents (to confirm whether the package itself is compromised vs. an overly-broad Defender signature) when this was paused. **Next step:** finish that inspection, then either pin `framer-motion`/`motion-dom` to a confirmed-clean version or replace the dependency if it's genuinely compromised.
